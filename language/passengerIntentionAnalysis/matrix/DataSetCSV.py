...
'''
from matrix.utils import checkExistence
import csv
...
def getCharDict(samples):
    odList = OrderedDict([('<pad>', 0)])
    vocab_size = len(odList.keys())
    data = ''
    for sent in samples:
        data += ''.join(sent.strip().split())
    list_chars = list(set(data))
    for token in list_chars:
        if token not in odList:
            odList[token] = vocab_size
            vocab_size += 1
    assert vocab_size == len(odList.keys()), 'Wrong number of vocab size.'
...
class DataSetCSV(object):
    def __init__(self, csv_file, train_data=None, flag='train'):
        ...
        if flag == 'test':
            assert isinstance(
                train_data,
                DataSetCSV), 'train_data is not an instance of DataSetCSV'
            ...
            self.char2id = train_data.char2id
            self.id2char = train_data.id2char
            self.char_vocab_size = train_data.char_vocab_size
    ...
    def _get_params(self):
        ...
        '''
        (self.userIntent2id, self.id2userIntent,
         self.userIntent_vocab_size) = getOrderedDict(
        ...
        (self.userTag2id, self.id2userTag,
         self.userTag_vocab_size) = getOrderedDict(
        ...
        (self.char2id, self.id2char,
         self.char_vocab_size) = getCharDict(self.userUtter_txt)
        ...
        self.userTagIntent_vocab_size = len(self.userTagIntent2id.keys())

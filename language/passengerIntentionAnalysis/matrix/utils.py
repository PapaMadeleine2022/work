...
def eval_slotTagging(tag_probs, mask_array, tag_trueLabel, O_id, mode):
    ...
    print('SlotTagging: mode={}, precision={:.4f}, recall={:.4f}, fscore={:.4f}, accuracy_frame={:.4f}'.format(mode, precision, recall, f1_score, accuracy_frame))

    return (precision, recall, f1_score, accuracy_frame)


def crf_eval_slotTagging(tag_probs, tag_trueLabel, num_classes, O_id, mode):
    ''' Evaluation for slot tagging.

        Inputs:
            tag_probs: shape = (sample_nb, maxlen_userUtter), predicted probs
            tag_trueLabel: shape is the same to tag_probs, indicator sparse matrix. If all zeros in one sample, the padding is assumed.
            id2tag: dict of id to tag string
            conll_fname: file name of .conll format that is suitable for conlleval.pl as input
        Outputs:
            ...
            accuracy at frame level.
    '''
    pred_tag_ids_masked = tag_probs
    true_tag_ids_masked = tag_trueLabel
    pred_tag_ids_noO = np.array(pred_tag_ids_masked)
    true_tag_ids_noO = np.array(true_tag_ids_masked)
    pred_tag_ids_noO[pred_tag_ids_masked == O_id] = 0  # exclude 'O' token
    true_tag_ids_noO[true_tag_ids_masked == O_id] = 0  # exclude 'O' token
    pred_tag_1hot_noO = to_categorical(pred_tag_ids_noO, num_classes)
    true_tag_1hot_noO = to_categorical(true_tag_ids_noO, num_classes)
    precision, recall, f1_score, _ = precision_recall_fscore_support(true_tag_1hot_noO.ravel(),
                                                                     pred_tag_1hot_noO.ravel(), beta=1.0, pos_label=1,
                                                                     average='binary')
    # true_tag_ids = (np.argmax(tag_trueLabel, axis=-1) + 1) * mask_array
    # [Q] Does all 'O's count in the denominator? temporally it is counted.
    accuracy_frame = calculate_FrameAccuracy(
        pred_tag_ids_masked, true_tag_ids_masked)
    print('SlotTagging: mode={}, precision={:.4f}, recall={:.4f}, fscore={:.4f}, accuracy_frame={:.4f}'.format(mode,
                                                                                                               precision,
                                                                                                               recall,
                                                                                                               f1_score,
                                                                                                               accuracy_frame))
...
def intent_accuracy(pred,true):
    '''calculate intent accuracy.
     for more than one intents, if pred in any one of them, consider pred is correct
     '''
    assert len(pred) == len(true),'number of pred and true is unequal'
    total = len(pred)
    pred_id = np.argmax(pred,-1) + 1
    correct = 0
    for i in range(total):
        if pred_id[i] in true[i]:
            correct += 1
    acc = correct * 1./ total
    return acc


def eval_intentPredict(intent_probs, intent_trueLabel, mode):
    ''' Inputs:
    ...
    print('Intent Prediction: mode={}, precision={:.4f}, recall={:.4f}, fscore={:.4f}, accuracy_frame={:.4f}, threshold={:.4f}'.format(mode, precision[max_idx], recall[max_idx], f1_score[max_idx], accuracy_frame, thresholds[max_idx]))

    return (precision[max_idx], recall[max_idx], f1_score[max_idx], accuracy_frame, thresholds[max_idx])

def eval_actionPredict(intent_probs, intent_trueLabel, mode):
    ''' Inputs:
            intent_probs: shape = (sample_nb, intent_vocab_size), predicted probs for intent prediction
            intent_trueLabel: shape = (sample_nb, intent_vocab_size), target binary matrix
        Output:
    ...
    indicator[intent_probs >= thresholds[max_idx]] = 1
    accuracy_frame = calculate_FrameAccuracy(indicator, intent_trueLabel)
    print('Action Prediction: mode={}, precision={:.4f}, recall={:.4f}, fscore={:.4f}, accuracy_frame={:.4f}, threshold={:.4f}'.format(mode, precision[max_idx], recall[max_idx], f1_score[max_idx], accuracy_frame, thresholds[max_idx]))
...
def crf_getNLUpred(tag_probs,id2tag, intent_probs, threshold, id2intent):
    tag_txt = list()
    pred_tag_ids = tag_probs
    for sample in pred_tag_ids:
        line_txt = [id2tag[tag_id].replace('tag-', '', 1)
                    for tag_id in sample if tag_id != 0]
        tag_txt.append(' '.join(line_txt))
...
def crf_getNLUframeAccuracy(tag_probs, tag_trueLabel, intent_probs, intent_trueLabel, threshold):
    pred_tag_ids = tag_probs
    indicator = np.zeros_like(intent_probs)
    indicator[intent_probs >= threshold] = 1
    pred = np.hstack((pred_tag_ids, indicator))
    true_tag_ids = tag_trueLabel
    true = np.hstack((true_tag_ids, intent_trueLabel))
    accuracy_frame = calculate_FrameAccuracy(pred, true)
    return accuracy_frame

...
if __name__=='__main__': 
  ...
  # get the 'ps_hosts' line @single 'ps' 
  ps_node = ps_node_list[0]
  ps_line = ps_node+':2222'  
  ps_line = '  --ps_hosts="'+ps_line+'" \\\n'
  print('>>')
  print(' %s' % (ps_line))
  print('>>')
  ...
  # generate each 'ps' script  @single 'ps' 
  script_name = os.path.join(script_dir,'ps_'+ps_node_list[0]+'.sh')
  print('>>ps : %s' % script_name)
  generate_ps(script_name, ps_line, wh_line, str(0))
  run_ps.append(script_name)

...
tf.flags.DEFINE_string('train_dir', '/tmp/imagenet_train',
                           """Directory where to write event logs """
                           """and checkpoint.""")
tf.flags.DEFINE_integer('max_steps', 1000000, 'Number of batches to run.')
tf.flags.DEFINE_string('subset', 'train', 'Either "train" or "validation".')
...
tf.flags.DEFINE_integer('num_replicas_to_aggregate', -1,
                            """Number of gradients to collect before """
                            """updating the parameters.""")
tf.flags.DEFINE_integer('save_interval_secs', 10 * 60,
                            'Save interval seconds.')
tf.flags.DEFINE_integer('save_summaries_secs', 180,
...
tf.flags.DEFINE_float('initial_learning_rate', 0.045,
    ...
    with slim.scopes.arg_scope(
      ...
      if is_chief:
        ...
        for l in losses + [total_loss]:
          ...
          tf.summary.scalar(loss_name + '__raw_', l)
      ...
      # Get chief queue_runners, init_tokens, which is used to synchronize replicas.
